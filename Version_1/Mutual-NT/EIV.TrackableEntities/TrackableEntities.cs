...
namespace EIV.TrackableEntities
{
    ...
    public sealed class TrackableEntities
    {
        ...
        /*public TrackableEntities(IList<T> initialList)
        {
            this.defaultList = initialList;
        ...
        //
        // Some magic here!!
        public void ProcessEntity<T>(object entity, TrackableEntity.Operation operation, string entityName = null) where T : class
        {
            if (entity == null)
            {
                return;
            ...
            if (!contains)
            {
                this.items.Add(new TrackableEntity(entity, operation, entityName));
            ...
            {
                if (thisEntity.OperationType == TrackableEntity.Operation.Current)
                {
            ...
            if (operation == TrackableEntity.Operation.Update)
            {
                if (thisEntity.OperationType == TrackableEntity.Operation.Insert)
                {
                    ...
                    this.items.Add(new TrackableEntity(entity, TrackableEntity.Operation.Insert, entityName));
            ...
            if (operation == TrackableEntity.Operation.Insert)
            {
                if (thisEntity.OperationType == TrackableEntity.Operation.Delete)
                {
                    this.items.Remove(thisEntity);
